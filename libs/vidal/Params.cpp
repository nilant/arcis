...
Params::Params(int seedRNG, Instance const& inst, int veh, std::vector<bool> const& required) 
{
	...
	int edgesTotal = 0;
	int arcsTotal = 0;
	int nodesTotal = inst.nvertices;

	std::vector<pair<int, int>> required_edges;
	std::vector<pair<int, int>> not_required_edges;
	std::vector<pair<int, int>> required_arcs;
	std::vector<pair<int, int>> not_required_arcs;

	required_edges.reserve(inst.nlinks);
	required_arcs.reserve(inst.nlinks);
	not_required_edges.reserve(inst.nlinks);
	not_required_arcs.reserve(inst.nlinks);

	for (int l = 0; l < inst.nreq_links; ++l) {
		int u = inst.links[l].first;
		int v = inst.links[l].second;
		if (inst.type(u, v) == EDGE) {
			if (required[l]) {
				ar_EdgesRequired++;
				required_edges.push_back({u+1, v+1});
			} else {
				not_required_edges.push_back({u+1, v+1});
			}
			edgesTotal++;
		} else { // it is an Arc
			if (required[l]) {
				ar_ArcsRequired++;
				required_arcs.push_back({u+1, v+1});
			} else {
				not_required_arcs.push_back({u+1, v+1});
			}
			arcsTotal++;
		}
	}

	for (int l = inst.nreq_links; l < inst.nlinks; ++l) {
		int u = inst.links[l].first;
		int v = inst.links[l].second;
		if (inst.type(u, v) == EDGE) {
			not_required_edges.push_back({u+1, v+1});
	...
	{
		int u = required_edges[k].first;
		int v = required_edges[k].second;
		cli[iCour].ar_nodesExtr0 = u;
		cli[iCour].ar_nodesExtr1 = v;
	...
	{
		int u = not_required_edges[k].first;
		side0 = u;
		int v = not_required_edges[k].second;
		side1 = v;
		myTravelCost = inst.trav_cost(u-1, v-1);
	...
	{
		int u = required_arcs[k].first; 
		int v = required_arcs[k].second; 
		cli[iCour].ar_nodesExtr0 = u;
		cli[iCour].ar_nodesExtr1 = v;
	...
	{
		int u = not_required_arcs[k].first;
		side0 = u;
		int v = not_required_arcs[k].second;
		side1 = v;
		myTravelCost = inst.trav_cost(u-1, v-1);
		ar_distanceNodes[side0][side1] = myTravelCost ;
	}
