...
//return costs and routes for vehicle
std::pair<std::vector<int>, vector < vector <std::pair<int,int> > > > Population::BestSolution () 
{
	vector <int> rout ;
		...
		for (int k=1 ; k <= params->nbDays ; k++)
		{
			...
			{	
				...
				if (!loc->routes[k][i].depot->suiv->estUnDepot)
				{
					...
					auto cost = loc->routes[k][i].depot->pred->seq0_i->evaluation(loc->routes[k][i].depot->pred->seq0_i,loc->routes[k][i].vehicle); // Printing the total cost of this route
